...
def generate_tokens(user_id):
    ...
    access_token = create_access_token(
        ...
        expires_delta=timedelta(
            hours=72
        ),  # Provisional, in the future we may want to shorten this and rely more on refresh tokens for security
        fresh=True,
    )
...
def grant_default_permissions(user_id):
    ...
    for permission_name in default_permissions:
        try:
            perm = UserPermission(
                user_id=user_id, permission_name=permission_name, is_granted=True
            )
            db.session.add(perm)
            count += 1
        except Exception:
...
def forgot_password():
    ...
    try:
        ...
        if user:
            # Generate reset token (JWT with 15 min expiration)
