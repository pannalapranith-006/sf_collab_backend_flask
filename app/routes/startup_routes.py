...
from app.utils.plans_utils import (
    can_create_project,
...
def has_startup_document_visibility_access(user_id, startup_document):
    ...
    print(
        "Has startup document visibility access check:",
        user_id,
        startup_document.parent_startup.creator_id,
    )
    print(
        "Type checks:", type(user_id), type(startup_document.parent_startup.creator_id)
    )
    if startup_document.parent_startup and int(
        startup_document.parent_startup.creator_id
    ) == int(user_id):
        return True
...
def get_startups():
    ...
    try:
        ...
        sort_by = request.args.get(
            "sort_by", "date", type=str
        )  # 'date', 'views', 'rating'
        query = Startup.query.filter(
            Startup.status != "deleted"
        )  # Exclude deleted startups

        if builder:
            print(
                "Builder filter applied - showing startups where user is a member but not creator"
            )
            query = query.join(
                StartupMember, StartupMember.startup_id == Startup.id
            ).filter(
        ...
        if sort_by == "views":
            ...
            avg_rating_subquery = (
                ...
                .group_by(StartupRating.startup_id)
                .subquery()
            )

            query = query.outerjoin(
                avg_rating_subquery, Startup.id == avg_rating_subquery.c.startup_id
            ).order_by(func.coalesce(avg_rating_subquery.c.avg_rating, 0).desc())
...
def get_startup(startup_id):
    current_user_id = request.args.get(
        "user_id", type=int
    )  # Optional user_id for access level determination
...
def register_startup():
    ...
    try:
        ...
        try:
            total_positions = calculate_total_positions(
                roles_data, data.get("positions", 0)
            )
        except Exception as e:
            ...
            if "logo" in files:
                ...
                if (
                    ...
                    if validate_file_size(logo_file):
                        ...
                        timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
                        unique_filename = (
                            f"{timestamp}_{uuid.uuid4().hex[:8]}_{filename}"
                        )
        ...
        try:
            if "banner" in files:
                ...
                if (
                    banner_file
                    and banner_file.filename != ""
                    and allowed_file(banner_file.filename)
                ):
            ...
            for doc_file in document_files:
                try:
                    if (
                        ...
                        and allowed_file(doc_file.filename)
                    ):
                        ...
                        file_size_mb = (
                            (os.path.getsize(doc_path) / (1024 * 1024))
                            if os.path.exists(doc_path)
                            else 0
                        )
                        can_upload, error_message = can_upload_file(
                            current_user_id, file_size_mb
                        )
                        if not can_upload:
                            if os.path.exists(doc_path):
                                os.remove(doc_path)
                            return error_response(error_message or "Upload failed", 403)
                    ...
                    print(
                        f"⚠️ [REGISTER_STARTUP] Individual document upload error: {str(e)}"
                    )
        ...
        try:
            existing_role = UserRole.query.filter_by(
                user_id=current_user_id, role="founder"
            ).first()
        ...
        try:
            ...
            if waitlist_user:
                ...
                last_activity_date = (
                    waitlist_user.last_activity_at.date()
                    if waitlist_user.last_activity_at
                    else None
                )

                if last_activity_date != today:
                    waitlist_user.add_points(
                        points=Waitlist.POINTS_PER_STARTUP, category="new_startup"
                    )
        except Exception as e:
        ...
        return success_response(
            {"startup": startup.to_dict(), "role": "founder"},
    ...
    try:
        ...
        if "name" in data:
            if (
                ...
                and Startup.query.filter_by(name=data["name"]).first()
            ):
                return error_response("Startup name already exists", 409)
        ...
        if "revenue" in data:
            startup.revenue = (
                float(data["revenue"]) if data["revenue"] is not None else 0.0
            )

        if "funding_amount" in data:
            startup.funding_amount = (
                float(data["funding_amount"])
                if data["funding_amount"] is not None
                else 0.0
            )
        ...
        if "burn_rate" in data:
            startup.burn_rate = (
                float(data["burn_rate"]) if data["burn_rate"] is not None else 0.0
            )

        if "runway_months" in data:
            startup.runway_months = (
                int(data["runway_months"]) if data["runway_months"] is not None else 0
            )

        if "valuation" in data:
            startup.valuation = (
                float(data["valuation"]) if data["valuation"] is not None else 0.0
            )
            ...
            if logo_file and allowed_file(logo_file.filename):
                if validate_file_size(logo_file):
                    logo_path = os.path.join(
                        startup_upload_dir, secure_filename(logo_file.filename)
                    )
                    logo_file.save(logo_path)
                    startup.logo_path = logo_path
                    startup.logo_url = f"/startups/{startup.id}/logo"
            ...
            if banner_file and allowed_file(banner_file.filename):
                if validate_file_size(banner_file):
                    banner_path = os.path.join(
                        startup_upload_dir, secure_filename(banner_file.filename)
                    )
                    banner_file.save(banner_path)
                    startup.banner_path = banner_path
                    startup.banner_url = f"/startups/{startup.id}/banner"
        ...
        db.session.commit()
        return success_response(
            {"startup": startup.to_dict()}, "Startup updated successfully"
        )
...
def add_startup_member(startup_id):
    ...
    try:
        ...
        db.session.commit()
        return success_response(
            {"member": member.to_dict()}, "Member added successfully"
        )
    except Exception as e:
        db.session.rollback()
...
def delete_startup(startup_id):
    ...
    if role == "none" or not role:
        print(
            f"❌ AUTHORIZATION FAILED - User {current_user_id} has role '{role}' for startup {startup_id}"
        )
...
def download_document(startup_id, document_id):
    ...
    document = StartupDocument.query.filter_by(
        id=document_id, startup_id=startup_id
    ).first_or_404()
...
def upload_document(startup_id):
    ...
    try:
        ...
        print(
            f"Uploading document: {document_file.filename}, visible_by: {request.form.get('visible_by', 'public')}"
        )
        ...
        return success_response(
            {"document": document.to_dict()}, "Document uploaded successfully"
        )
    except Exception as e:
...
def get_startup_names():
    ...
    member_startups = StartupMember.query.filter_by(
...
def get_join_requests(startup_id):
    ...
    if not can_manage_members(startup_id, current_user_id):
        return error_response(
            "Only the startup owner or founders can view join requests", 403
        )
...
def send_join_request(startup_id):
    ...
    try:
        ...
        if not current_user:
            ...
            return error_response("User not found", 404)
        membership = StartupMember.query.filter_by(
            startup_id=startup_id, user_id=current_user_id
        ).first()
        if membership:
        ...
        if existing:
            print(
                f"User {current_user_id} already has pending request {existing.id} for startup {startup_id}"
            )
            return error_response(
                "You already have a pending request for this startup", 409
            )
        ...
        print(
            f"Creating join request for user {user.first_name} {user.last_name} to startup {startup.name}"
        )
        ...
        try:
            ...
            managers = StartupMember.query.filter(
                ...
                (StartupMember.role.in_(["owner", "founder"]))
                | (StartupMember.admin == True),
                StartupMember.is_active == True,
            ).all()
            print(
                f"Notifying {len(managers)} managers about new join request:",
                [f"{m.first_name} {m.last_name}" for m in managers],
            )
...
@startups_bp.route(
    "/<int:startup_id>/join-requests/<int:request_id>/accept", methods=["POST"]
)
@jwt_required()
def accept_join_request(startup_id, request_id):
    ...
    if not can_manage_members(startup_id, current_user_id):
        return error_response(
            "Only the startup owner or founders can accept join requests", 403
        )

    join_request = JoinRequest.query.filter_by(
        id=request_id, startup_id=startup_id, status=JoinRequestStatus.pending
    ).first()
    ...
    try:
        ...
        try:
            ...
            notify_access_granted(
                user_id=user_id_to_notify, resource=startup.name, resource_id=startup.id
            )
        except Exception as e:
        ...
        return success_response(
            {
                ...
                "new_member": {
                    ...
                    "joinedAt": (
                        ...
                        if hasattr(new_member, "joined_at")
                        else None
                    ),
                },
            },
...
@startups_bp.route(
    "/<int:startup_id>/join-requests/<int:request_id>/reject", methods=["POST"]
)
@jwt_required()
def reject_join_request(startup_id, request_id):
    ...
    if not can_manage_members(startup_id, current_user_id):
        return error_response(
            "Only the startup owner or founders can reject join requests", 403
        )
    ...
    try:
        ...
        return success_response(
            {"message": "Join request rejected successfully", "request_id": request_id}
        )
...
def cancel_my_join_request(request_id):
    ...
    join_request = JoinRequest.query.filter_by(
        id=request_id, user_id=current_user_id, status=JoinRequestStatus.pending
    ).first()
...
def has_startup_management_access(user_id, startup_id):
    ...
    allowed_roles = {UserRoles.admin.value, "manager", UserRoles.founder.value, "owner"}
...
def remove_startup_member(startup_id, member_id):
    ...
    if member.user_id == current_user:
        return error_response(
            "You cannot remove yourself. Use the leave startup endpoint instead.", 400
        )
    ...
    if member_user_id:
        try:
            notify_removed_from_startup(
                user_id=member_user_id, startup_name=startup.name, startup_id=startup.id
            )
        except Exception as e:
...
@startups_bp.route(
    "/<int:startup_id>/members/<int:member_id>/promote", methods=["POST"]
)
@jwt_required()
...
@startups_bp.route(
    "/<int:startup_id>/members/<int:member_id>/change-role", methods=["POST"]
)
@jwt_required()
...
def get_startup_invitations(startup_id):
    ...
    if not can_manage_members(startup_id, current_user_id):
        return error_response(
            "Only the startup owner or founders can view invitations", 403
        )
...
def create_startup_invitation(startup_id):
    ...
    if not can_manage_members(startup_id, current_user_id):
        return error_response(
            "Only the startup owner or founders can send invitations", 403
        )
    ...
    existing_member = StartupMember.query.filter_by(
        startup_id=startup_id, user_id=invited_user_id
    ).first()
    if existing_member:
    ...
    try:
        ...
        return success_response(
            {"invitation": invitation.to_dict()}, "Invitation sent successfully", 201
        )
...
@startups_bp.route(
    "/<int:startup_id>/invitations/<int:invitation_id>/accept", methods=["POST"]
)
@jwt_required()
...
@startups_bp.route(
    "/<int:startup_id>/invitations/<int:invitation_id>/reject", methods=["POST"]
)
@jwt_required()
...
@startups_bp.route(
    "/<int:startup_id>/invitations/<int:invitation_id>", methods=["DELETE"]
)
@jwt_required()
def cancel_startup_invitation(startup_id, invitation_id):
    ...
    if not can_manage_members(startup_id, current_user_id):
        return error_response(
            "Only the startup owner or founders can cancel invitations", 403
        )

    invitation = StartupInvitation.query.filter_by(
        id=invitation_id, startup_id=startup_id, status=InvitationStatus.pending
    ).first()
...
def rate_startup(startup_id):
    ...
    try:
        ...
        existing_rating = StartupRating.query.filter_by(
            user_id=current_user_id, startup_id=startup_id
        ).first()
        ...
        return success_response(
            {"message": "Rating submitted successfully", "rating": float(rating)}
        )
    except Exception as e:
        db.session.rollback()
...
def get_startup_launch_data(startup_id):
    ...
    try:
        ...
        if not isinstance(response_text, str):
            return error_response("Invalid AI response format", 500)
...
def refine_startup_description(startup_id):
    ...
    try:
        ...
        return success_response(
            {"refined_description": response_text.strip(), "tokens_used": tokens_used}
        )
...
def suggest_startup_roles(startup_id):
    ...
    try:
        ...
        return success_response(
            {"roles": roles_data.get("roles", {}), "tokens_used": tokens_used}
        )
