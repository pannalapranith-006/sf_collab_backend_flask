...
def create_app(config_name=None):
    ...
    # JWT Configuration
    app.config["JWT_SECRET_KEY"] = os.getenv("JWT_SECRET_KEY") or app.config.get(
        "SECRET_KEY"
    )
    ...
    app.config["SMTP_USE_TLS"] = os.getenv("MAIL_USE_TLS", "true").lower() in [
        "true",
        "1",
        "t",
    ]
    ...
    app.config["SMTP_DEFAULT_SENDER"] = os.getenv(
    ...
    def log_response(response):
        if request.path in ("/favicon.ico", "/health"):
            return response
        if request.method == "OPTIONS" and not response.headers.get(
...
""")
    ...
    if app.config.get("GOOGLE_CLIENT_ID") and app.config.get("GOOGLE_CLIENT_SECRET"):
        ...
        print(
            "⚠  OAuth not initialized (missing GOOGLE_CLIENT_ID/GOOGLE_CLIENT_SECRET)"
        )
