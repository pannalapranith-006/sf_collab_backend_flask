...
class DriveFile(db.Model):
    ...
    # Folder placement — no FK (avoids circular dependency with DriveFolder)
    ...
    # Optional relation IDs (denormalised for fast lookup)
    ...
    superseded_by = db.relationship(
        "DriveFile",
        remote_side=[id],
        foreign_keys=[superseded_by_file_id],
    )
